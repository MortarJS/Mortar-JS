 ...
 * @type {ReactComponent}
 *
 * @prop {!object}   options      The selectable options.
 * @prop {function=} actionMethod A function that is called when an option is toggled
 *
 * @memberOf Filters
 * @see {@link Filters}
 *
...
var MultipleFilterOptions = React.createClass({
	...
	buildOptionGroups: function() {
		...
		for (var group in this.props.options) {
			if (this.props.options.hasOwnProperty(group)) {
				groups.push(
					<div>
	 ...
	 * @memberOf Filters.MultipleFilterOptions
	 *
	 * @param {object} header The header label for the checkbox
	 * @return {JSX}
	 */
	buildOptions: function(header) {
		...
		for (var x in this.props.options[header]) {
			if (! this.props.options[header][x]) {
				return false;
			}
	 ...
	 * @memberOf Filters.MultipleFilterOptions
	 *
	 * @param  {string} formKey The form key for the filters form
	 * @param  {object} filters The filters we're passing to be requested
	 * @return {null}
	 */
	...
	setFilters: function() {
		...
		for (var g in data) {
			...
			data[g].forEach(function(datum) {
				...
				if (! datum.value.hasOwnProperty(key)) {
					return;
				}
