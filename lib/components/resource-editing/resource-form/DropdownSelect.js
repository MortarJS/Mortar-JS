...
var DropdownSelect = React.createClass({
	...
	buildSelectOptions: function() {
		...
		return options.map(function(option, index) {
			...
			return (
				<li onClick={self.handleAction.bind(null, 'select', option)} className={'dropdown-option list-group-item'}> {displayLabel} </li>
			);
		});
	},
	...
	render: function() {
		...
		var dropdownStyle = function() {
			...
			if (self.refs.dropdownDrawer) {
				drawer = self.refs.dropdownDrawer.getDOMNode();
				height = 0;
