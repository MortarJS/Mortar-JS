...
var TypeAheadInput = React.createClass({
	...
	componentWillReceiveProps: function(nextProps) {
		if (this.props.options != nextProps.options) {
			...
			if (this.props.debug) {
				...
				console.table(nextProps.source);
			}

      if (typeof nextProps.options === 'object') {
        console.log("NEW WHERE SEARCH", this.state.typeAhead.query, nextProps.options)
  			typeAhead.queryResults = this.where(nextProps.options, this.state.typeAhead.query);
  			this.setState({typeAhead: typeAhead});
  		} else if (nextProps.options === 'remote') {
			...
			return value.map(function (option, index) {
				...
				return (
					<div key={index} className="selected-option">
						{display}
						<a href="javascript:void(0)" className="typeahead-unselect icon-times pull-right"
	...
	render: function () {
		...
		return (
			<div className={`typeahead-select mortar-typeahead ${this.props.formKey}-${this.props.fieldKey}`}>
				<div className={formGroupClasses}>
					...
					<Input formKey={this.props.fieldKey + '-query'} fieldKey='query' bindResource={this.state.typeAhead} changeCallback={this.handleSearchChange}
						placeholder={this.props.placeholder} type="text" value={this.state.typeAhead.query} />
				</div>
