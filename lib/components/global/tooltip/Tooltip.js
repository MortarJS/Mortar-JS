...
var Tooltip = React.createClass({
	...
	setWidth: function(width, orientation, styles) {
		try {
			...
			console.warn(`A digit is required when specifying width.  Your input: "${width}".  Falling back to 150px.`);
		}
	},
	...
	setHeight: function(height, orientation, styles) {
		try {
			...
			if (orientation === "left" || orientation === "right") {
				...
				styles.marginTop = -Math.ceil(heightValue / 2);
			}

		} catch (e) {
			console.warn(`A digit is required when specifying height.  Your input: "${height}".  Falling back to 50px.`);
		}
	},
